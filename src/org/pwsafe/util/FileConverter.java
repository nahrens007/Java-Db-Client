/*
 * $Id: FileConverter.java 488 2013-12-11 23:22:25Z roxon $
 * 
...
public class FileConverter {
	...
	/**
	 * Converts <code>oldFile</code> to the latest supported version, currently
	 * version 2. If the file is already the latest format no new file is
	 * created and the reference is simply returned as-is.
	 * 
	 * @param oldFile the file to be converted.
	 * 
	 * @return A file in the latest format containing the data from
	 *         <code>oldFile</code>.
	 * 
	 * @throws IOException
	 * @throws PasswordSafeException
	 */
	public static PwsFile convertToLatest(PwsFile oldFile) throws IOException,
	 ...
	 * @throws PasswordSafeException
	 */
	public static PwsFile convertV1ToV2(PwsFileV1 oldFile) throws IOException,
			PasswordSafeException {
		PwsFileV2 newFile;
		PwsRecordV1 oldRec;
		PwsRecord newRec;
		...
		final PwsFileStorage oldStorage = (PwsFileStorage) oldFile.getStorage();
		final PwsFileStorage newStorage = new PwsFileStorage(makeNewFilename(
				oldStorage.getFilename(), "v2-"));
		newFile.setStorage(newStorage);

		for (final Iterator iter = oldFile.getRecords(); iter.hasNext();) {
	 ...
	 * @throws PasswordSafeException
	 */
	public static PwsFile convertV2ToV1(PwsFileV2 oldFile) throws IOException,
			PasswordSafeException {
		PwsFileV1 newFile;
		PwsRecordV2 oldRec;
		PwsRecord newRec;
	...
	/**
	 * Makes a new filename from the given filename and prefix. The prefix is
	 * prepended to the name. For example if <code>filename</code> is
	 * "C:\Program Files\Java\PasswordSafe\MyPasswords.dat" and
	 * <code>prefix</code> is "V2-", the new filename would be "C:\Program
	 * Files\Java\PasswordSafe\V2-MyPasswords.dat"
	 * 
	 * @param filename
	 * @param prefix
	 * @return
	...
	private static String makeNewFilename(String filename, String prefix) {
		...
		final StringBuilder sb = new StringBuilder(path.length() + prefix.length() + name.length());
		final String newName = sb.append(path).append(prefix).append(name).toString();
