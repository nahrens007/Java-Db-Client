...
public class PwsRecordV3 extends PwsRecord {
	...
	private static final Object[] VALID_TYPES = new Object[] {
			...
			new Object[] { Integer.valueOf(PASSWORD_MOD_TIME), "PASSWORD_MOD_TIME",
					PwsTimeField.class },
			new Object[] { Integer.valueOf(LAST_ACCESS_TIME), "LAST_ACCESS_TIME",
					PwsTimeField.class },
			new Object[] { Integer.valueOf(PASSWORD_LIFETIME), "PASSWORD_LIFETIME",
					PwsTimeField.class },
			new Object[] { Integer.valueOf(PASSWORD_POLICY_DEPRECATED), "PASSWORD_POLICY_OLD",
			...
			new Object[] { Integer.valueOf(PASSWORD_HISTORY), "PASSWORD_HISTORY",
					PwsStringUnicodeField.class },
			new Object[] { Integer.valueOf(PASSWORD_POLICY), "PASSWORD_POLICY",
					PwsStringUnicodeField.class },
			new Object[] { Integer.valueOf(PASSWORD_EXPIRY_INTERVAL), "PASSWORD_EXPIRY_INTERVAL",
					PwsStringUnicodeField.class }, };
	...
	protected boolean isValid() {
		...
		final PwsField idField = getField(V3_ID_STRING);

		if (idField != null) {
			LOG.debug1("Ignoring record " + toString());
			return false;
		}
		return true;
	}
	...
	protected class ItemV3 extends Item {
		public ItemV3(PwsFileV3 file) throws EndOfFileException, IOException {
			...
			try {
				...
				file.readBytes(data);
				final byte[] hash = file.hasher.doFinal();
			...
			final byte[] remainingDataInRecord = Util.getBytes(rawData, 5, 11);
			if (length <= 11) {
				Util.copyBytes(Util.getBytes(remainingDataInRecord, 0, length), data);
			} else if (length > 11) {
				final int bytesToRead = length - 11;
				...
				for (int i = 0; i < blocksToRead; i++) {
					...
					if (i == blocksToRead - 1) {
						// last block, do magic
						nextBlock = Util.getBytes(nextBlock, 0, bytesToRead
	...
	protected void saveRecord(PwsFile file) throws IOException {
		...
		for (final Iterator<Integer> iter = getFields(); iter.hasNext();) {
			int type;
			PwsField value;
			...
			if (LOG.isDebug2Enabled()) {
				LOG.debug2("Writing field " + type + " (" + ((Object[]) VALID_TYPES[type])[1]
						+ ") : \"" + value.toString() + "\"");
			}

			writeField(file, value);

			final PwsFileV3 fileV3 = (PwsFileV3) file;
	...
	protected void writeField(PwsFile file, PwsField field, int type) throws IOException {
		...
		int bytesToPad = 0;
		final int calcWriteLen = lenBlock.length + dataBlock.length;
		...
		final byte[] dataToWrite = Util.mergeBytes(lenBlock, dataBlock);

		for (int i = 0; i < (dataToWrite.length / 16); i++) {
			final byte[] nextBlock = Util.getBytes(dataToWrite, i * 16, 16);
	...
	public String toString() {
		...
		for (final Map.Entry<Integer, PwsField> entry : attributes.entrySet()) {
