/*
 * $Id: PwsRecordV1.java 488 2013-12-11 23:22:25Z roxon $
 * 
...
public class PwsRecordV1 extends PwsRecord implements Comparable {
	...
	/**
	 * The string that, if present in the title, indicates that the default
	 * username is to be used for this record.
	 */
	...
	/**
	 * The string that separates the title and username when it is a composite
	 * field.
	 */
	...
	static {
		try {
			// Must be done here as they could theoretically thow an exception,
			// though in
	 ...
	 */
	PwsRecordV1(PwsFile file) throws EndOfFileException, IOException {
	...
	private boolean equals(PwsRecordV1 other) {
		return (getField(NOTES).equals(other.getField(NOTES))
				...
				&& getField(TITLE).equals(other.getField(TITLE)) && getField(USERNAME).equals(
				other.getField(USERNAME)));
	}
	...
	protected void saveRecord(PwsFile file) throws IOException {
		...
		if (getField(USERNAME).toString().trim().length() == 0) {
			...
			title = new PwsStringField(TITLE, getField(TITLE).toString() + SplitString
