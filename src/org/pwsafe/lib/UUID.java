/*
 * $Id: UUID.java 488 2013-12-11 23:22:25Z roxon $
 * 
...
public class UUID implements Comparable, Serializable {
	...
	public UUID() {
		...
		Util.putIntToByteArray(uuid, time_lo, 0);
		Util.putIntToByteArray(uuid, time_hi, 4);

		uuid[0] = Util.newRand();
		uuid[1] = Util.newRand();
		uuid[2] = (byte) (Util.newRand() & 0x07);
		uuid[8] = (byte) (Util.newRand() & 0x3f);
		uuid[9] = Util.newRand();
		uuid[10] = Util.newRand();
		uuid[11] = Util.newRand();
		uuid[12] = Util.newRand();
		uuid[13] = Util.newRand();
		uuid[14] = Util.newRand();
		uuid[15] = Util.newRand();
	}

	/**
	 * Construct the UUID from the 16 byte array <code>uuid</code>.
	 * 
	 * @param aUuid the 16 bytes to use as the UUID.
	 */
	public UUID(byte[] aUuid) {
		if (aUuid.length != uuid.length) {
			throw new IllegalArgumentException();
		}
		System.arraycopy(aUuid, 0, uuid, 0, uuid.length);
	}
	...
	/**
	 * Compares this <code>UUID</code> to another returning a value less than
	 * zero if <code>this</code> is "less than" <code>other</code>, zero if
	 * they're equal and greater than zero if <code>this</code> is
	 ...
	 *         <code>other</code>, zero if they're equal and greater than zero
	 *         if <code>this</code> is "greater than" <code>other</code>.
	 */
	...
	public int compareTo(UUID other) {
		for (int ii = 0; ii < uuid.length; ++ii) {
			final int b1 = uuid[ii] & 0x0ff;
			final int b2 = other.uuid[ii] & 0x0ff;
	...
	public byte[] getBytes() {
		return Util.cloneByteArray(uuid);
	}
	...
	/**
	 * Converts <code>uuid</code> into human-readable form. The string has the
